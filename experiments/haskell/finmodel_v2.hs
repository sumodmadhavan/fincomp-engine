...
main = do
  let !params = FinancialParams
      ...
      !initialRate = 320.0
      !targetProfit = 3000000.0

  start <- getPOSIXTime

  let !initialCumulativeProfit = calculateFinancials initialRate params
      (# !optimalRate, !iterations #) = goalSeek targetProfit params initialRate
  ...
  printf "Initial Warranty Rate: %.2f\n" initialRate
  printf "Initial Cumulative Profit: %.2f\n" initialCumulativeProfit
  printf "\nOptimal Warranty Rate to achieve %.2f profit: %.7f\n" targetProfit optimalRate
  printf "Number of iterations: %d\n" iterations
  printf "\nFinal Cumulative Profit: %.2f\n" finalCumulativeProfit
